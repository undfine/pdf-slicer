...
- **Raster images**: Transparent images (with a soft mask) are page-rendered with the alpha channel composited. Small opaque images (<15% of page area, e.g. logos) are extracted directly from their PDF xref for maximum fidelity.
...
- **Image-to-image seams**: Abutting or composited images that share an edge are not cut apart.
